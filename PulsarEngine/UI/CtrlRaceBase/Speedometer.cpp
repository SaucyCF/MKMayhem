...
u32 CtrlRaceSpeedo::Count() {
    if(Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC3, MISC_RADIO_SOM) == RACESETTING_SOM_DISABLED) return 0;
...
void CtrlRaceSpeedo::Create(Page& page, u32 index, u32 count) {
    ...
    for(int i = 0; i < count; ++i) {
        ...
        char variant[0x20];
        int pos = i;
        if(count == 1 && Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC3, MISC_RADIO_SOM) == RACESETTING_SOM_RIGHT) pos = 1;
...
void CtrlRaceSpeedo::OnUpdate() {
    ...
    const u8 digits = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC3, MISC_SCROLL_SOM);
