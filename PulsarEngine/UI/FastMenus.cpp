...
static void FasterMenusOnSceneChange(SectionMgr* sectionMgr, u32 delay, u32 color) {
    if(Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC, MISC_RADIO_FASTMENUS) == MENUSETTING_FASTMENUS_ENABLED) delay = 0;
...
static void FasterMenuPatchTransitionDelay() {
    const u8 val = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC, MISC_RADIO_FASTMENUS);
