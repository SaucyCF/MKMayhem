...
//DKW Dev Note: Fixes by Optilizer
...
namespace Pulsar {
    ...
    winningCourse(PULSARID_NONE), selectedCourse(PULSARID_FIRSTREG), lastSelectedCup(PULSARCUPID_FIRSTREG), lastSelectedCupButtonIdx(0), isAlphabeticalLayout(false)
    {
        ...
        // First 128 tracks remain in original order
        for (int i = 0; i < 128; ++i) {
        ...
        for (int i = 0; i < 64; ++i) {
            lastTrackIndices[i] = i + 128;  // Initialize with sequential indices
        }
        ...
        for (int i = 0; i < 64; ++i) {
            alphabeticalArray[128 + i] = lastTrackIndices[i];
        }
    ...
    void CupsConfig::SetWinning(PulsarId id, u32 variantIdx) {
        bool VSTTsOnline1 = Pulsar::OTTSETTING_ONLINE_DISABLED;
        bool VSTTsOnline2 = Pulsar::OTTSETTING_ONLINE_DISABLED;
        bool VSTTsLocal1 = Pulsar::OTTSETTING_OFFLINE_DISABLED;
        bool VSTTsLocal2 = Pulsar::OTTSETTING_OFFLINE_DISABLED;
        ...
        if (!IsReg(id)) {
            ...
            VSTTsOnline1 = System::sInstance->IsContext(Pulsar::PULSAR_MODE_OTT) ? Pulsar::OTTSETTING_ONLINE_NORMAL : Pulsar::OTTSETTING_ONLINE_DISABLED;
            VSTTsOnline2 = System::sInstance->IsContext(Pulsar::PULSAR_MODE_OTT) ? Pulsar::OTTSETTING_ONLINE_FEATHER : Pulsar::OTTSETTING_ONLINE_DISABLED;
            VSTTsLocal1 = System::sInstance->IsContext(Pulsar::PULSAR_MODE_OTT) ? Pulsar::OTTSETTING_OFFLINE_NORMAL : Pulsar::OTTSETTING_OFFLINE_DISABLED;
            VSTTsLocal2 = System::sInstance->IsContext(Pulsar::PULSAR_MODE_OTT) ? Pulsar::OTTSETTING_OFFLINE_FEATHER : Pulsar::OTTSETTING_OFFLINE_DISABLED;
        }
            if (mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE || mode == MODE_BATTLE || mode == MODE_PUBLIC_BATTLE || mode == MODE_PRIVATE_BATTLE || VSTTsOnline1 == Pulsar::OTTSETTING_ONLINE_NORMAL || VSTTsOnline2 == Pulsar::OTTSETTING_ONLINE_FEATHER || VSTTsLocal1 == Pulsar::OTTSETTING_OFFLINE_NORMAL || VSTTsLocal2 == Pulsar::OTTSETTING_OFFLINE_FEATHER) variantIdx = 0;
    ...
    void CupsConfig::ToggleCTs(bool enabled) {
        ...
        if(RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_HOST || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_NONHOST) {
            isRegs = System::sInstance->IsContext(PULSAR_REGS) ? DKWSETTING_TRACKSELECTION_REGS : false;
        }
        if(RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_VS_WW || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_BT_WW || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_JOINING_WW) {
            isRegs = DKWSETTING_TRACKSELECTION_REGS;
        }
        if(isRegs == DKWSETTING_TRACKSELECTION_REGS) {
    ...
    PulsarId CupsConfig::RandomizeTrack() const {
        ...
        Random random;
        u32 pulsarId;
        u32 isCTOnly = DKWSETTING_TRACKSELECTION_ALL;
        u32 isSITOnly = DKWSETTING_TRACKSELECTION_ALL;
        u32 isRegsOnly = DKWSETTING_TRACKSELECTION_ALL;
        if (RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_HOST || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_NONHOST) {
            if (System::sInstance->IsContext(PULSAR_CTS)) isCTOnly = DKWSETTING_TRACKSELECTION_CTS;
            if (System::sInstance->IsContext(PULSAR_SITS)) isSITOnly = DKWSETTING_TRACKSELECTION_SITS;
            if (System::sInstance->IsContext(PULSAR_REGS)) isRegsOnly = DKWSETTING_TRACKSELECTION_REGS;
        }
        if (RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_NONE) {
            if (settings.GetUserSettingValue(Settings::SETTINGSTYPE_DKW2, SETTINGDKW_TRACKS) == DKWSETTING_TRACKSELECTION_CTS) isCTOnly = DKWSETTING_TRACKSELECTION_CTS;
            if (settings.GetUserSettingValue(Settings::SETTINGSTYPE_DKW2, SETTINGDKW_TRACKS) == DKWSETTING_TRACKSELECTION_SITS) isSITOnly = DKWSETTING_TRACKSELECTION_SITS;
            if (settings.GetUserSettingValue(Settings::SETTINGSTYPE_DKW2, SETTINGDKW_TRACKS) == DKWSETTING_TRACKSELECTION_REGS) isRegsOnly = DKWSETTING_TRACKSELECTION_REGS;
        }
        else if (isCTOnly == DKWSETTING_TRACKSELECTION_CTS && isRegsOnly != DKWSETTING_TRACKSELECTION_REGS || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_VS_REGIONAL || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_JOINING_REGIONAL) pulsarId = random.NextLimited(128) + 0x100;
        else if (isSITOnly == DKWSETTING_TRACKSELECTION_SITS && isRegsOnly != DKWSETTING_TRACKSELECTION_REGS) pulsarId = random.NextLimited(64) + 0x100 + 128;
        else if (isRegsOnly == DKWSETTING_TRACKSELECTION_REGS) pulsarId = random.NextLimited(32);
    ...
    */
    ...
    PulsarCupId CupsConfig::GetNextCupId(PulsarCupId pulsarId, s32 direction) const {
        ...
        if (isCTOnly == DKWSETTING_TRACKSELECTION_CTS && isRegsOnly != DKWSETTING_TRACKSELECTION_REGS || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_VS_REGIONAL || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_JOINING_REGIONAL) {
            const u32 countCT = 32;
            const u32 minCT = countCT < 8 ? 8 : 0;
            const u32 nextIdxCT = ((idx + direction + countCT) % countCT) + minCT;
        ...
        } else if (isSITOnly == DKWSETTING_TRACKSELECTION_SITS && isRegsOnly != DKWSETTING_TRACKSELECTION_REGS) {
            const u32 countSIT = 16;
            const u32 lastCupIndex = this->GetTotalCupCount() - 1;
            const u32 startIdx = 40;
            const u32 nextIdxSIT = startIdx + ((idx - startIdx + direction + countSIT) % countSIT);
            if(!this->hasRegs && nextIdxSIT < 8) return static_cast<PulsarCupId>(nextIdxSIT + countSIT + 0x38);
            return ConvertCup_IdxToPulsarId(nextIdxSIT);
        } else {
