...
static bool IsCustomItemEnabled(u32 item) {
    u32 settingIdx;
    Pulsar::Context2 hostBit;
    switch (item) {
        case 21: settingIdx = Pulsar::ITEMS_BOO;                  hostBit = Pulsar::PULSAR_BOO;            break;
        case 22: settingIdx = Pulsar::ITEMS_FEATHER;              hostBit = Pulsar::PULSAR_FEATHER;        break;
        case 23: settingIdx = Pulsar::ITEMS_TRIPLE_FIB;           hostBit = Pulsar::PULSAR_TRIPLEFIB;      break;
        case 24: settingIdx = Pulsar::ITEMS_SHROOM_STAR;          hostBit = Pulsar::PULSAR_SHROOMSTAR;     break;
        case 25: settingIdx = Pulsar::ITEMS_GREEN_SHELL_MUSHROOM; hostBit = Pulsar::PULSAR_SHELLMUSHROOM;  break;
        case 26: settingIdx = Pulsar::ITEMS_BOB_OMB_MUSHROOM;     hostBit = Pulsar::PULSAR_BOBOMBMUSHROOM; break;
        default: return true;
    }
    ...
    if (controller != nullptr && system != nullptr) {
        ...
        const bool isOnlineGuest = (controller->roomType == RKNet::ROOMTYPE_FROOM_NONHOST)
                                && (sub.localAid != sub.hostAid);
        if (isOnlineGuest) {
            return (system->netMgr.hostContext2 & (1 << hostBit)) != 0;
        }
    }
    return Pulsar::Settings::Mgr::Get().GetSettingValue(
        Pulsar::Settings::SETTINGSTYPE_ITEMS, settingIdx) == Pulsar::DKWSETTING_ITEM_ENABLED;
}
...
extern "C" void BuildItemSlotBinary() {
    ...
    for (u32 t = 0; t < ItemProbs::TABLE_COUNT; t++) {
        ...
        for (u32 item = 0; item < ItemProbs::ITEM_COUNT; item++) {
            const u8* row = def.data + item * cols;
            const bool enabled = IsCustomItemEnabled(item);
            for (u32 col = 0; col < cols; col++) {
                compiledItemSlotBin[offset++] = enabled ? row[col] : 0;
            }
        }
    }
    ...
    for (u32 t = 0; t < ItemProbs::TABLE_COUNT; t++) {
        ...
        if (t <= 4) {
            for (u32 item = 0; item < ItemProbs::ITEM_COUNT; item++) {
                const bool enabled = IsCustomItemEnabled(item);
                for (u32 col = 0; col < cols; col++) {
                    bool isActive = enabled && (item <= 18 || item == 21 || item == 22 || item == 23 || item == 24 || item == 25 || item == 26);
            ...
            for (u32 item = 0; item < ItemProbs::ITEM_COUNT; item++) {
                ...
                const bool enabled = IsCustomItemEnabled(item);
                for (u32 col = 0; col < cols; col++) {
                    compiledUnknownItemsBin[offset++] = enabled ? row[col] : 0;
                }
            }
        }
    }
...
static void InvalidateItemSlotCache() {
    compiledItemSlotLen = 0;
    compiledUnknownItemsLen = 0;
}
RaceLoadHook InvalidateItemSlot(InvalidateItemSlotCache);
...
extern "C" bool ExpandedCapacityCheck(ItemId id) {
    if (id >= 21 && id <= 26 && !IsCustomItemEnabled((u32)id)) return false;
    if (id == BOO) {
