...
static void DisableRaceMusic(Audio::SinglePlayer& singlePlayer, u32 soundId, s16 delay) {
    const bool isEnabled = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC, MISC_RADIO_MUSIC) == MENUSETTING_MUSIC_DEFAULT;
...
static void DisableMenuMusic(Audio::SinglePlayer& singlePlayer, u32 soundId, s16 delay) {
    const bool isEnabled = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_MISC, MISC_RADIO_MUSIC) != MENUSETTING_MUSIC_DISABLE_ALL;
    if(isEnabled) singlePlayer.PlaySound(soundId, delay);
}
