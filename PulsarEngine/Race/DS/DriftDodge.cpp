...
// Returns true if the player is airborne from a feather jump (jumpPadType 7)
static bool IsFeatherAirborne(Kart::Collision* collision) {
	if (collision == nullptr) return false;
	Kart::Pointers* pointers = collision->pointers;
	if (pointers == nullptr) return false;
	Kart::Status* status = pointers->kartStatus;
	if (status == nullptr) return false;
	return (status->bitfield0 & 0x40000000) && status->jumpPadType == 0x7 && status->airtime > 0;
}
...
static int HandleBlueShellCollision(Kart::Collision* collision, void* itemObj) {
	...
	// Feather airborne = always immune to blue shell
	if (IsFeatherAirborne(collision)) {
		return static_cast<int>(NO_DAMAGE);
	}

	// SMT dodge = immune to blue shell (Mayhem mode only, checked in CanDodgeBlueShell)
...
kmBranch(0x805731b0, HandleBlueShellCollision);
