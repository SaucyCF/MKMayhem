...
static void SustainStartBoostChain() {
	if(Pulsar::System::sInstance == nullptr) return;
	if(!Pulsar::System::sInstance->IsContext(PULSAR_MAYHEM)) return;
...
static void ForceTwoSecondStartBoost(Kart::Movement* movement, s32 startBoostFrames) {
	...
	if(System::sInstance == nullptr || !System::sInstance->IsContext(PULSAR_MAYHEM)) {
		movement->ApplyStartBoost(startBoostFrames);
		return;
	}
